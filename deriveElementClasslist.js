 ...
 * const classesCell = deriveElementClasslist(div)
 *
 ...
 */
export function deriveElementClassList(element) {
  const classList = Cell.source(Array.from(element.classList));

  const observer = new MutationObserver(() => {
    classList.value = Array.from(element.classList);
  });
